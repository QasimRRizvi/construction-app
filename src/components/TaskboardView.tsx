...
const TaskboardView = () => {
  ...
  useEffect(() => {
    const fetchTasksAndChecklists = async () => {
      ...
      const checklistResults = await db.checklists
        .find({ selector: { taskId: { $in: tasks.map(t => t.id) } } })
        .exec();
    ...
    if (user?.id) fetchTasksAndChecklists();
    else navigate('/');
  }, [user, navigate, setChecklistForTask, setTasks]);
  ...
  const groupedTasks = useMemo(
    ...
    [tasks, checklistsByTask, getStatusesByTask],
  );
  ...
  const StatusBadge = ({ status }: { status: ChecklistStatus }) => {
    ...
    return (
      <Badge className={`${config.bgColor} border-0`}>
        ...
        <span className={`text-xs font-extrabold ml-2 ${config.color}`}>{config.label}</span>
      </Badge>
    );
  };
  ...
  const TaskCard = ({
    ...
    <div
      className={`${isBoard ? 'mb-3' : 'mb-2'} border border-gray-300 rounded-lg hover:shadow-md transition-shadow duration-200 bg-white`}
    >
      <div className={`${isBoard ? 'p-4' : 'p-3'}`}>
        <div className="flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <h3
              className={`font-medium ${isBoard ? 'text-sm' : 'text-base'} text-gray-900 truncate`}
            >
              {task.title}
            </h3>
      ...
      {statusOrder.map(status => (
        <div key={status} className="space-y-2">
          <div className="flex items-center space-x-2">
            ...
            <Badge className="text-xs">{groupedTasks[status].length}</Badge>
          </div>
        ...
        return (
          <div key={status} className="w-80 shrink-0 border border-gray-300 rounded-lg p-4">
            <div className="flex items-center justify-between mb-4 bg-gray-200 p-3 rounded-lg">
              <div className="flex items-center space-x-2">
                <Circle className={`w-3 h-3 fill-current ${config.dotColor}`} />
                <h3 className="font-bold text-gray-900">{status}</h3>
              </div>
              <Badge className="text-xs">{groupedTasks[status].length}</Badge>
            </div>
            <div className="space-y-3 max-h-[calc(100vh-310px)] overflow-y-auto">
              {groupedTasks[status].map(task => (
  ...
  return (
    <div className="px-4 py-6 text-white">
      ...
      <div className="max-w-7xl mx-auto ">
        ...
        {/* Content */}
        <div className="transition-all duration-300 ease-in-out overflow-auto h-[calc(100vh-180px)] bg-gray-50 rounded-lg p-4 border border-gray-300">
