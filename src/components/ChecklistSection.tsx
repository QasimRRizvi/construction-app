    ...
    <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg z-10 min-w-48">
      {Object.entries(statusConfig).map(([status, config]) => (
        <Button
          ...
          color="secondary"
          className={clsx(
            'w-full px-3 py-2 text-left flex items-center gap-2 first:rounded-t-md last:rounded-b-md focus:outline-none focus:bg-gray-50 !justify-start',
      ...
      {open && (
        <div id="checklist-content" className="space-y-4">
                    ...
                    <>
                      <div className="flex items-center gap-2">
                        <label
                          ...
                          className={clsx('block text-md cursor-pointer flex-1 text-gray-900 ', {
                            'line-through text-gray-500': item.status === ChecklistStatus.Done,
                            [statusConfig[item.status].color]:
                              statusConfig[item.status].label === ChecklistStatus.Blocked,
                          })}
                        >
                          {item.label}
                        </label>
                      ...
                      <div id={`item-${item.id}-status`} className="flex items-center gap-1">
                        <Circle
                          className={`w-2 h-2 fill-current ${statusConfig[item.status].dotColor}`}
                        />
                        <span
          ...
          <div className="flex items-center gap-3 pt-2 border-t border-gray-100">
            <Button onClick={addNewItem} className="!p-1" aria-label="Add new checklist item">
